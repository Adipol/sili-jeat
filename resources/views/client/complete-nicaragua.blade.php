<x-client-layout>
    <h1 class="text-2xl font-bold">Información de la lista completa (Nicaragua)</h1>
    ...
    <article class="card mb-6">
        <div class="card-body bg-gray-100 text-sm">
            ...
            <div>
                @if ($complete)
                    @livewire('client.all-download-nicaragua', ['download_all' => $complete], key($complete->id))
                @endif
            </div>
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    @foreach ($codes as $index => $code)
                                        <tr class="{{ $index % 2 == 0 ? 'bg-blue-50' : 'bg-blue-100' }}">
                                            <td class="font-small text-sm text-gray-900">
                                                <div class="ml-4">
                                                    {{ $code->code }}
                                                </div>
                                            </td>
